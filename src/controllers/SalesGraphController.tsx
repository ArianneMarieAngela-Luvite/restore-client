...
//   // Combine sales and prediction data
// const combinePredictionData = () => {
//   if (!parsedData || !salesPredictionData) return;

//   const combinedDataMap = {}; // Initialize as an object, not an array

//   // Start by copying parsedData into combinedDataMap
//   parsedData.forEach((entry) => {
//     combinedDataMap[entry.month] = { ...entry }; // Copy each month entry into combinedDataMap
//   });

//   console.log("combi before predictions", combinedDataMap);

//   // Iterate through sales prediction data
//   salesPredictionData.forEach((prediction) => {
//     const [year, month] = prediction.next_month.split('-'); // Split the next_month into year and month
//     const predictionSales = parseFloat(prediction.prediction); // Parse prediction sales as a number

//     // Get month name from month number
//     const monthName = getMonthNameFromNumber(month);

//     // If the month exists in combinedDataMap, add the prediction
//     if (combinedDataMap[monthName]) {
//       combinedDataMap[monthName][year] = predictionSales;
//     } else {
//       // If the month doesn't exist in sales data, create a new entry for it
//       combinedDataMap[monthName] = { month: monthName, [year]: predictionSales };
//     }
//   });

//   console.log("combi after predictions", combinedDataMap);

//   // Convert back to array format if necessary
//   const combinedDataArray = Object.values(combinedDataMap);

//   // Update the state with the combined data
//   setParsedData(combinedDataArray);
...
// // Add the combinePredictionData to the useEffect once sales and prediction data are ready
// useEffect(() => {
//   if (parsedData && salesPredictionData.length > 0) {
//     combinePredictionData();
//   }
// }, [salesPredictionData]);
 ...
 // Fetch sales data
  ...
  // // Format and parse sales data
  // const handleParsedData = (data: any[]) => {
  //   const formattedDataMap: { [key: string]: any } = {};
